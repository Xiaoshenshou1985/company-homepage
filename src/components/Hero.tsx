...
function AnimatedCounter({ end, suffix = '', prefix = '', duration = 2000, decimals = 0 }: CounterProps) {
  ...
  return (
    <div ref={ref} className="text-4xl md:text-5xl font-bold text-primary-500 tabular-nums">
...
export default function Hero() {
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center hero-pattern overflow-hidden bg-white">
      {/* Decorative elements */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-20">
        ...
        <div className="animate-fade-in-up" style={{ animationDelay: '0.2s', opacity: 0 }}>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary-500/20 bg-primary-500/5 mb-8">
            ...
            <span className="text-primary-500 text-sm tracking-wider">HONGBANG GROUP</span>
          </div>
        </div>
        ...
        <h1 className="animate-fade-in-up text-5xl md:text-7xl lg:text-8xl font-bold mb-6 tracking-tight" style={{ animationDelay: '0.4s', opacity: 0 }}>
          <span className="text-surface-900">宏邦</span>
          <span className="text-primary-500">集团</span>
        </h1>

        <p className="animate-fade-in-up text-xl md:text-2xl text-surface-500 mb-4 tracking-[0.3em]" style={{ animationDelay: '0.6s', opacity: 0 }}>
          科技驱动 · 产业协同 · 价值创造
        </p>

        <p className="animate-fade-in-up text-base text-surface-400 mb-16 max-w-2xl mx-auto" style={{ animationDelay: '0.8s', opacity: 0 }}>
        ...
        <div className="animate-fade-in-up grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 max-w-4xl mx-auto" style={{ animationDelay: '1s', opacity: 0 }}>
          {metrics.map((metric, i) => (
            <div key={i} className="p-6 rounded-2xl bg-white border border-surface-200/50 shadow-sm hover:shadow-md hover:border-primary-200 transition-all duration-300">
              <AnimatedCounter
                end={metric.value}
              ...
              />
              <div className="text-surface-400 text-sm mt-2">{metric.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="animate-fade-in-up mt-20" style={{ animationDelay: '1.4s', opacity: 0 }}>
          <a href="#overview" className="inline-flex flex-col items-center text-surface-300 hover:text-primary-500 transition-colors">
